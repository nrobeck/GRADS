...
public class ModelTests {
	...
	DataManager my_dm = new DataManager(null,
		"src/resources/studentsTest.txt", null, null);
	CheckResultDetails A_CheckResultDetails = new CheckResultDetails();
	A_CheckResultDetails.setGPA((float) 3.0);
	A_CheckResultDetails.setCourses(my_dm.getStudentData("nguy0621")
		.getCoursesTaken());
	...
	CourseTaken A_CourseTaken = new CourseTaken(sample_course, sample_term,
		Grade.A);
	CourseTaken B_CourseTaken = new CourseTaken(sample_course, sample_term,
		Grade.B);
	...
	MilestoneSet B_MilestoneSet = new MilestoneSet(
		Milestone.PRELIM_COMMITTEE_APPOINTED, new Term(Semester.SPRING,
			2014));
	...
	Professor B_Professor = new Professor("Minerva", "McGonagall",
		Department.MATH);
	...
	RequirementCheckResult A_RequirementCheckResult = new RequirementCheckResult(
		"A", true, Arrays.asList("err1", "err2"));
	RequirementCheckResult B_RequirementCheckResult = new RequirementCheckResult(
