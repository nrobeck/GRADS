...
public class SummaryBuilder {
     ...
     * 
     * @param d
     *            - a pointer to a DataManager object
     * @param t
     *            - a pointer to a TranscriptHandler object
     */
    ...
    /**
     * Creates a graduate progress summary based on the student's transcript and
     * courses to simulate.
     * 
     * @param studentID
     *            The id of the student
     * @param simCourses
     *            A list of courses to simulate
     * @return A completed summary of a student's progress towards his/her
     *         graduate degree
     */
    public ProgressSummary createStudentSummary(String studentID,
	    List<CourseTaken> simCourses) {
	...
	for (CourseTaken c : record.getCoursesTaken()) {
	    courses.add(c);
	}
	courses = removeDuplicateCourses(courses);
	...
	summary.setRequirementCheckResults(checkPlanRequirements(
		record.getDegreeSought(), courses, record.getMilestonesSet(),
		simulate));
     ...
     * 
     * @param plan
     *            - the plan that will be checked
     * @param courses
     *            - the courses that the student has taken
     * @param milestones
     *            - a list of milestones of the student
     * @return a list of Requirement Check Results based on the plan/student
     *         progress
     */
    private List<RequirementCheckResult> checkPlanRequirements(Degree degree,
	    List<CourseTaken> courses, List<MilestoneSet> milestones,
	    boolean simulate) {
	// create the needed elements
	List<RequirementCheckResult> retVal = new ArrayList<RequirementCheckResult>();

	List<CourseTaken> trimmedList = removeDuplicateCourses(courses);
     ...
     * @param simulate
     *            indicates whether there are simulated courses or not
     * @return
     */
    private RequirementCheckResult checkColloquium(List<CourseTaken> courses,
	    Degree degree, boolean simulate) {
     ...
     */
    private RequirementCheckResult checkPlanBProject(List<CourseTaken> courses,
	    Degree degree, boolean simulate) {
	RequirementCheckResult result = new RequirementCheckResult(
		"PLAN_B_PROJECT", false);
     ...
     * @return
     */
    private RequirementCheckResult checkResearchIntro(
	    List<CourseTaken> courses, Degree degree, boolean simulate) {
	RequirementCheckResult result = new RequirementCheckResult(
		"INTRO_TO_RESEARCH");
	CheckResultDetails details = new CheckResultDetails();
	List<CourseTaken> tempCourseList = new ArrayList<CourseTaken>();
     ...
     * @return
     */
    private List<RequirementCheckResult> checkGPARequirementCheckResults(
	    List<CourseTaken> courses, Degree degree) {
	// create the needed elements
	List<RequirementCheckResult> retVal = new ArrayList<RequirementCheckResult>();
	...
	if (gpa < minGPA) {
	    result.setPassed(false);
	    result.addErrorMsg("Overall GPA is below the minimum");
	}
     ...
     * @return
     */
    private List<RequirementCheckResult> checkCreditRequirements(
	    List<CourseTaken> courses, Degree degree, boolean simulate) {
	List<RequirementCheckResult> retVal = new ArrayList<RequirementCheckResult>();
	RequirementCheckResult result = new RequirementCheckResult(
		"TOTAL_CREDITS");
	...
	for (CourseTaken c : courses) {
	    ...
	    if (!((!(degree == Degree.PHD)
		    && (c.getCourse().getId().equals("csci8888")) || c
		    .getCourse().getId().equals("csci8777")))) {
		...
		if (passedCourse(c, false, simulate)) {
			...
			if (isGraduteLevel(c.getCourse(), "csci")) {
			    tempCourseList.add(c);
			    csciCredits += Integer.parseInt(c.getCourse()
				    .getNumCredits());
			    totalCredits += Integer.parseInt(c.getCourse()
				    .getNumCredits());
			}
		    }
	...
	if (degree == Degree.PHD) {
		...
		if (!(c.getCourse().getId().contains("csci"))) {
			...
			if (isGraduteLevel(c.getCourse(), "other")) {
			    tempCourseList.add(c);
			    tempCredits += Integer.parseInt(c.getCourse()
				    .getNumCredits());
			}
		    }
		}
	...
	if (degree == Degree.MS_A) {
		...
		if (!(c.getCourse().getId().equals("csci8888") || c.getCourse()
			.getId().equals("csci8777"))) {
		    // make sure that the course was passed
		    if (passedCourse(c, false, simulate)) {
			...
			tempCourseList.add(c);
			courseCredits += Integer.parseInt(c.getCourse()
				.getNumCredits());
		    }
		    // Any C or higher non-csci course counts for total credits
		    else {
     ...
     * @return
     */
    private RequirementCheckResult checkThesisRequirements(
	    List<CourseTaken> courses, Degree degree, boolean simulate) {
	RequirementCheckResult result;
	List<CourseTaken> tempCourseList = new ArrayList<CourseTaken>();
	CheckResultDetails details = new CheckResultDetails();
	...
	if (degree == Degree.PHD) {
		...
		if (c.getCourse().getId().equals("csci8888")) {
			...
			try {
			    Integer temp = Integer.parseInt(c.getCourse()
				    .getNumCredits());
			    if (temp != null) {
				tempCourseList.add(c);
				tempSum += temp; // add number of credits to
						 // total
			    }
			} catch (Exception e) {
			    // keep going
     ...
     * @return
     */
    private RequirementCheckResult checkPHDLevelCsciCourses(
	    List<CourseTaken> courses, Degree degree, boolean simulate) {
	RequirementCheckResult result;
	List<CourseTaken> tempCourseList = new ArrayList<CourseTaken>();
	CheckResultDetails details = new CheckResultDetails();
     ...
     * @return
     */
    private RequirementCheckResult checkBreadthRequirements(
	    List<CourseTaken> courses, Degree degree, boolean simulate) {
	RequirementCheckResult result;
	CheckResultDetails details = new CheckResultDetails();
	;
	...
	if (gpa >= minGPA) {
	    ...
	    result.setPassed(false);
	    result.addErrorMsg("Breadth GPA is below the required minimum: "
		    + minGPA);
	}
    ...
    /**
     * Creates a list of RequirmentCheckResults based on what milestones have
     * been completed
     * 
     * @param milestones
     *            the milestones that a student has completed
     * @param degree
     *            the degree of the student
     * @return a degree-specific list of milestone results
     */
    private List<RequirementCheckResult> checkMilestones(
	    List<MilestoneSet> milestones, Degree degree) {
	List<RequirementCheckResult> retVal = new ArrayList<RequirementCheckResult>();
	RequirementCheckResult result;
	...
	case PHD:
	    planMilestones = new Milestone[] {
		    ...
		    Milestone.WRITTEN_PE_APPROVED, Milestone.ORAL_PE_PASSED,
		    Milestone.DPF_SUBMITTED, Milestone.DPF_APPROVED,
		    Milestone.THESIS_COMMITTEE_APPOINTED,
		    Milestone.PROPOSAL_PASSED,
		    Milestone.GRADUATION_PACKET_REQUESTED,
		    Milestone.THESIS_SUBMITTED, Milestone.THESIS_APPROVED,
     ...
     * 
     * @param courses
     *            - the list of courses to derive a GPA from
	...
	// divide sum of gpa totals by number for credits taken to get gpa and
	// then return it
	return sum / credits;
    }
     ...
     * 
     * @param course
     *            - the course that a student has taken
     * @param passFail
     *            - tells if S grades are allowable
     ...
     * 
     * @param course
     *            the course that is a candidate for being graduate level
     * @param department
	...
	if (course.getId().length() > 4
		&& (department.equals("other") || course.getId().contains(
			department))) {
     ...
     */
    private List<CourseTaken> removeDuplicateCourses(
	...
	for (CourseTaken c : coursesTaken) {
		...
		if (c.equals(d)) {
		    ...
		    if (c.getGrade().numericValue() >= d.getGrade()
			    .numericValue()) {
			retVal.remove(d); // remove the old best grade
		    } else {
			addOK = false; // There already exists a better grade
				       // course
		    }
		}
	    }
    ...
    /**
     * Return the progress summary based solely upon a student's transcript and
     * no simulated courses. Calls createStudentSummary with null for simulated
     * courses
     * 
     * @see createStudentSummary method used to perform all creation
     * @param userId
     *            user id of the student whose progress summary is to be
     *            generated.
